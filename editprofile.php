...
{
echo "<script>window.location.assign('doctorlogin.php')</script>";
}
?>
...
<?php
    if(isset($_POST['btnreg']))
    {
      ...
      $p=md5($password);
              //Query start
              //MD5('".$userpass."')
              $checkname="SELECT username FROM user_registration WHERE username='$username' and uId!=$id and Status='doctor'";
              ...
              {
                $checkname="SELECT phoneNumber FROM user_registration WHERE phoneNumber='$phoneNumber' and uId!=$id and Status='doctor'";
                $checknamequery=mysqli_query($con,$checkname) or die(mysqli_error());
                $checknamefetch=mysqli_fetch_assoc($checknamequery);
                ...
                {
                    $sql="update user_registration set Name='$name',phoneNumber='$phoneNumber',Address='$address',City='$city',username='$username' where uId=$id";
